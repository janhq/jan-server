...
func init() {
	registry.RegisterSchemaForAutoMigrate(&ConnectorConnection{})
	registry.RegisterSchemaForAutoMigrate(&ConnectorOAuthState{})
	registry.RegisterSchemaForAutoMigrate(&ConnectorAuditLog{})
}
...
type ConnectorConnection struct {
	...
	UserID                string         `gorm:"type:varchar(64);not null;index:idx_connector_connections_user_type"`
...
type ConnectorAuditLog struct {
	...
	UserID        string `gorm:"type:varchar(64);not null;index:idx_connector_audit_log_user"`
	ConnectorType string `gorm:"type:varchar(32);not null"`
