...
import (
	...
	domain "jan-server/mono/apps/backend/internal/domain/artifact"
...
func (r *PostgresRepository) List(ctx context.Context, filter *domain.Filter) ([]*domain.Artifact, int64, error) {
	query := r.db.WithContext(ctx).Model(&dbschema.Artifact{}).
		Preload("Response").
		Preload("Plan").
		Preload("Parent")
	...
	var entityList []dbschema.Artifact
	if err := query.
		Order(orderClause).
...
func (r *PostgresRepository) ListVersions(ctx context.Context, artifactID string) ([]*domain.Artifact, error) {
	...
	var rootEntity dbschema.Artifact
	if err := r.db.WithContext(ctx).
		Select("id").
	...
	// Find all versions
	var entities []dbschema.Artifact
	if err := r.db.WithContext(ctx).
		Preload("Response").
		Preload("Plan").
		Preload("Parent").
...
func (r *PostgresRepository) Delete(ctx context.Context, id string) error {
	if err := r.db.WithContext(ctx).
		Where("public_id = ?", id).
		Delete(&dbschema.Artifact{}).Error; err != nil {
		return platformerrors.NewError(
			ctx,
			platformerrors.LayerRepository,
			platformerrors.ErrorTypeDatabaseError,
...
func (r *PostgresRepository) MarkOldVersionsNotLatest(ctx context.Context, newVersionID string, parentID string) error {
	// Get internal IDs
	var parentEntity dbschema.Artifact
	if err := r.db.WithContext(ctx).
		Select("id").
...
func mapArtifactToEntity(artifact *domain.Artifact, responseID uint, planID, parentID *uint) *dbschema.Artifact {
...
func mapArtifactFromEntity(entity *dbschema.Artifact) *domain.Artifact {
...
func (r *PostgresRepository) hydrateArtifactRefs(ctx context.Context, artifact *domain.Artifact, entity *dbschema.Artifact) error {
