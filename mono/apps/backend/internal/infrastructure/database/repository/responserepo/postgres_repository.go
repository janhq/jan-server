...
import (
	...
	domain "jan-server/mono/apps/backend/internal/domain/response"
...
func (r *PostgresRepository) Update(ctx context.Context, resp *domain.Response) error {
	...
	if err := r.db.WithContext(ctx).Model(&dbschema.Response{ID: resp.ID}).Updates(entity).Error; err != nil {
		return platformerrors.NewError(
			ctx,
			platformerrors.LayerRepository,
			platformerrors.ErrorTypeDatabaseError,
...
func (r *PostgresRepository) RecordExecutions(ctx context.Context, responseID uint, executions []tool.Execution) error {
	...
	rows := make([]dbschema.ToolExecution, 0, len(executions))
	for _, exec := range executions {
		...
		rows = append(rows, dbschema.ToolExecution{
...
func mapToEntity(resp *domain.Response) (*dbschema.Response, error) {
...
func mapFromEntity(entity *dbschema.Response, resp *domain.Response) error {
	resp.ID = entity.ID
