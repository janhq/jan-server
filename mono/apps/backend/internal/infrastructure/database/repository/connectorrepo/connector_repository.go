...
func (r *Repository) FindByUserAndType(ctx context.Context, userID string, connectorType connector.ConnectorType) (*connector.Connection, error) {
...
func (r *Repository) FindAllByUser(ctx context.Context, userID string) ([]*connector.Connection, error) {
...
func (r *Repository) Delete(ctx context.Context, userID string, connectorType connector.ConnectorType) error {
	err := r.db.WithContext(ctx).
		Where("user_id = ? AND connector_type = ?", userID, string(connectorType)).
...
func (r *Repository) FindAuditLogsByUser(ctx context.Context, userID string, limit int) ([]*connector.AuditLog, error) {
	if limit <= 0 {
		limit = 100
	}
