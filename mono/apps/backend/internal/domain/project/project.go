...
type Project struct {
	...
	UserID string       `json:"-"`      // Internal user ID
...
type ProjectRepository interface {
	...
	GetByPublicIDAndUserID(ctx context.Context, publicID string, userID string) (*Project, error)
	GetByNameAndUserID(ctx context.Context, name string, userID string) (*Project, error)
	ListByUserID(ctx context.Context, userID string, pagination *query.Pagination) ([]*Project, int64, error)
...
func NewProject(publicID string, userID string, name string, instruction *string) *Project {
	now := time.Now()
