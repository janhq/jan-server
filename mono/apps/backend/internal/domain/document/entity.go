...
type DocumentContentRepository interface {
	...
	GetByMediaObjectID(ctx context.Context, mediaObjectID string, userID string) (*DocumentContent, error)
...
func NewDocumentContent(publicID string, userID string, mediaObjectID, filename, mimeType string, fileSize int64) *DocumentContent {
	now := time.Now()
...
func NewProjectFile(publicID string, projectID uint, documentContentID uint, createdBy string, displayOrder int) *ProjectFile {
	now := time.Now()
