...
func (s *Service) createKeyInternal(ctx context.Context, usr *user.User, name string, requestedTTL time.Duration, isSystem bool) (*APIKey, string, error) {
	...
	if s.keycloak != nil && !isSystem {
		if err := s.keycloak.StoreAPIKeyHash(ctx, usr.ID, record.ID, keyHash); err != nil {
			s.logger.Warn().Err(err).Str("user_id", usr.ID).Msg("failed to store api key in keycloak")
...
func (s *Service) GetOrCreateSystemKey(ctx context.Context, usr *user.User, requestedTTL time.Duration) (*APIKey, string, error) {
	...
	if err != nil {
		s.logger.Warn().Err(err).Str("user_id", usr.ID).Msg("failed to find existing system key, will create new one")
	}
...
func (s *Service) RevokeKey(ctx context.Context, usr *user.User, keyID string) error {
	...
	if s.keycloak != nil && !key.IsSystem {
		if usr.ID != "" {
			if err := s.keycloak.RemoveAPIKeyHash(ctx, usr.ID, key.ID); err != nil {
				s.logger.Warn().
					Err(err).
					Str("key_id", key.ID).
					Str("user_id", usr.ID).
			...
			s.logger.Warn().
				Str("key_id", key.ID).
				Str("user_id", usr.ID).
				Msg("api key revoked but user ID missing; unable to remove from keycloak")
		}
	}
...
func (s *Service) ValidateAPIKey(ctx context.Context, apiKey string) (*keycloak.APIKeyUserInfo, error) {
	...
	usr, err := s.userRepo.GetByID(ctx, key.UserID)
	if err != nil {
	...
	// Step 4: Double-check user status in Keycloak (if configured)
	if s.keycloak != nil && usr.ID != "" {
		keycloakUser, err := s.keycloak.GetUserBySubject(ctx, usr.ID)
		if err != nil {
			s.logger.Error().
				Err(err).
				Str("user_id", usr.ID).
	...
	// If Keycloak is not configured, return basic user info
	s.logger.Debug().
		Str("key_id", key.ID).
		Str("user_id", usr.ID).
