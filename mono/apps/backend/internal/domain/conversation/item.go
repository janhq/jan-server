...
// GetRole returns the item's role, defaulting to ItemRoleUser if not set.
...
func (i *Item) GetContent() []Content {
	return i.Content
}

// GetStatus returns the item's status, defaulting to ItemStatusCompleted if not set.
...
// GetContentAsString extracts the text content from the item as a single string.
func (i *Item) GetContentAsString() string {
	if len(i.Content) == 0 {
		return ""
	}
	var parts []string
