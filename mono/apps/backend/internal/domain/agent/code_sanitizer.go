...
// NormalizeSandboxFilePaths rewrites common file output paths to a writable sandbox directory.
func NormalizeSandboxFilePaths(code string) string {
	if code == "" {
		return code
	}

	// Helper to create patterns for both single and double quotes
	// Go's regexp doesn't support backreferences, so we create separate patterns
