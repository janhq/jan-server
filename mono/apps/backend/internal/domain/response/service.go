...
func (s *ServiceImpl) createSync(ctx context.Context, params CreateParams) (*Response, error) {
	...
	existingItems, err := s.conversationItems.FindByConversationID(ctx, conv.ID)
	if err != nil {
	...
	if err := s.conversationItems.BulkCreate(ctx, newItems); err != nil {
...
func (s *ServiceImpl) ListConversationItems(ctx context.Context, publicID string) ([]ConversationItem, error) {
	...
	items, err := s.conversationItems.FindByConversationID(ctx, *resp.ConversationID)
	if err != nil {
		return nil, err
	}
...
func (s *ServiceImpl) convertInputToMessages(conversationID uint, startingSeq int, input interface{}) ([]llm.ChatMessage, []*conversation.Item, error) {
	...
	case string:
		...
		item := newConversationItem(conversationID, startingSeq, msg)
		convoItems = append(convoItems, &item)
	case []interface{}:
		for _, raw := range v {
			...
			messages = append(messages, msg)
			item := newConversationItem(conversationID, startingSeq+len(convoItems), msg)
			convoItems = append(convoItems, &item)
		}
...
func contentToLLM(content []conversation.Content) interface{} {
	if len(content) == 0 {
		return nil
	}
	// Extract text content for LLM messages
	var parts []string
	for _, c := range content {
		if c.TextString != nil {
			parts = append(parts, *c.TextString)
		} else if c.Text != nil && c.Text.Text != "" {
			parts = append(parts, c.Text.Text)
		} else if c.OutputText != nil && c.OutputText.Text != "" {
			parts = append(parts, c.OutputText.Text)
		}
	}
	if len(parts) == 0 {
		return nil
