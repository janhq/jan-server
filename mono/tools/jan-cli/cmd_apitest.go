...
This is a lightweight CLI API test runner that supports essential
features for Jan Server testing: running collections, setting
...
// Test helper functions (inlined from testhelpers package)
...
func runCollection(collectionFile string, envMap map[string]string) error {
	...
	if autoAuth != "" {
		...
		if autoAuth == "admin" || autoAuth == "both" {
			...
			if kcUser != "" && kcPass != "" {
				if token, err := adminLogin(kcURL, kcUser, kcPass); err == nil && token != "" {
		...
		if token, err := guestLogin(gatewayURL); err == nil && token != "" {
			...
			if fallbackURL != "" && fallbackURL != gatewayURL {
				if token, err := guestLogin(fallbackURL); err == nil && token != "" {
					envMap["guest_access_token"] = token
					if envMap["access_token"] == "" {
						envMap["access_token"] = token
					}
	...
	if autoModels && envMap["access_token"] != "" {
		gatewayURL := firstNonEmpty(envMap["gateway_url"], envMap["kong_url"], "http://localhost:8080")
		if model, err := getDefaultModel(gatewayURL, envMap["access_token"]); err == nil && model != "" {
			envMap["model_id"] = model
			envMap["model_id_encoded"] = getModelEncoded(model)
...
func buildURL(urlInterface interface{}, envMap map[string]string) string {
	...
	case map[string]interface{}:
		...
		if raw, ok := v["raw"].(string); ok {
			urlStr := replaceVariables(raw, envMap)
					...
					if varMap, ok := varInterface.(map[string]interface{}); ok {
						if key, ok := varMap["key"].(string); ok {
							if value, ok := varMap["value"].(string); ok {
								...
								urlStr = strings.ReplaceAll(urlStr, ":"+key, replacedValue)
							}
						}
					}
				}
...
func ensureDefaultTokens(envMap map[string]string) {
	...
	gatewayURL := strings.TrimSpace(envMap["gateway_url"])
	if gatewayURL == "" {
		gatewayURL = "http://localhost:8080"
	}
	loginURL := strings.TrimSuffix(gatewayURL, "/") + "/auth/guest-login"
...
func printResults(results []TestResult, totalDuration time.Duration) {
	...
	if failed == 0 {
		...
		fmt.Printf("[FAIL] Some tests failed\n\n")
	}
}
